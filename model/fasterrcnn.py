...
class FasterRCNN(tf.keras.Model):
    ...
    def __init__(self, n_class, pool_size,backbone = None):
        super(FasterRCNN, self).__init__()
        self.n_class = n_class
        if backbone is None:
            self.extractor = Extractor()
        elif backbone == 'imagenet':
            self.extractor = tf.keras.applications.MobileNetV2(include_top=False, weights='imagenet')
        else:
            self.extractor = tf.saved_model.load('coco')
